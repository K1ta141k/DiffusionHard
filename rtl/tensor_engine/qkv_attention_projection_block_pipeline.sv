...
module qkv_attention_projection_block_pipeline #(
    ...
    wire [3:0] canvas_group_read_head,canvas_group_read_group;
    ...
    qkv_attention_multihead_canvas_pipeline #(
...
`ifndef SYNTHESIS
        if (rst_n && producer_array_valid && projection_array_valid)
            $error("attention producer and output projection requested together");
`endif
    end
