...
module attention_score_group_softmax #(
    ...
    localparam [2:0] STATE_SOFTMAX_START = 3'd1;
    localparam [2:0] STATE_SOFTMAX_FEED = 3'd2;
    localparam [2:0] STATE_SOFTMAX_WAIT = 3'd3;
    localparam [2:0] STATE_GROUP_OUTPUT = 3'd4;

    reg [2:0] state;
    reg [GROUP_WIDTH-1:0] active_group;
    reg [KEY_TILE_WIDTH-1:0] expected_key_tile;
    reg [1:0] active_query;
    reg [6:0] feed_key;
    ...
    wire softmax_start = (state == STATE_SOFTMAX_START)
        && softmax_start_ready;
    wire softmax_score_ready;
    wire softmax_score_valid = (state == STATE_SOFTMAX_FEED)
        && (feed_key < 64);
    wire softmax_probability_valid;
    wire [5:0] softmax_probability_key;
    ...
    assign probability_group_valid = (state == STATE_GROUP_OUTPUT);
    ...
    attention_softmax_row_q16 #(
        ...
        .start(softmax_start),
        .head_in(4'd0),
        .query_in({active_group, active_query}),
        .start_ready(softmax_start_ready),
        .score_valid(softmax_score_valid),
        .score_ready(softmax_score_ready),
        .score_q10(score_memory[active_query][feed_key[5:0]]),
        .probability_valid(softmax_probability_valid),
        .probability_ready(1'b1),
        .head_out(),
        .query_out(),
        .key_out(softmax_probability_key),
        .probability_q16(softmax_probability),
        .busy(),
        .done(softmax_done)
    );
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            end else if (state == STATE_SOFTMAX_START && softmax_start_ready) begin
                feed_key <= 0;
                state <= STATE_SOFTMAX_FEED;
            end else if (state == STATE_SOFTMAX_FEED
                         && softmax_score_valid && softmax_score_ready) begin
                if (feed_key == 63)
                    state <= STATE_SOFTMAX_WAIT;
                else
                    feed_key <= feed_key + 1'b1;
            end else if (state == STATE_SOFTMAX_WAIT && softmax_done) begin
                if (active_query == M_LANES-1) begin
                    state <= STATE_GROUP_OUTPUT;
                end else begin
                    active_query <= active_query + 1'b1;
                    state <= STATE_SOFTMAX_START;
                end
            end else if (state == STATE_GROUP_OUTPUT
                         && probability_group_ready) begin
                state <= STATE_LOAD;
                done <= 1'b1;
            end

            if (softmax_probability_valid)
                probability_memory[active_query][softmax_probability_key] <=
                    softmax_probability;
        end
    end
...
`ifndef SYNTHESIS
        if (rst_n && state == STATE_LOAD && score_tile_valid
            ...
            $error("attention score tile arrived out of order");
        if (rst_n && state == STATE_LOAD && score_tile_valid
            && expected_key_tile != 0 && score_tile_group != active_group)
            $error("attention score group changed within a matrix");
`endif
    end
