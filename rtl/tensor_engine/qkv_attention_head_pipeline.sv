...
module qkv_attention_head_pipeline #(
    ...
    assign array_request_tag = staging_phase
        ? staging_array_tag : {4'b0, head_array_tag};
    ...
    attention_head_pipeline #(
        .INTERNAL_MAC(INTERNAL_MAC), .SEPARATE_VALUE_LOAD_ADDRESS(1),
        .LUT_FILE(LUT_FILE)
    ) head (
        ...
        .array_request_valid(head_array_valid),
        .array_request_clear(head_array_clear),
        .array_request_last(head_array_last), .array_request_tag(head_array_tag),
        .array_request_activations(head_array_activations),
        .array_request_weights(head_array_weights),
        .array_response_valid(!staging_phase && array_response_valid),
        .array_response_tag(array_response_tag[3:0]),
        .array_response_accumulators(array_response_accumulators),
...
`ifndef SYNTHESIS
        if (rst_n && staging_array_valid && head_array_valid)
            $error("QKV and attention requested the shared array together");
`endif
    end
