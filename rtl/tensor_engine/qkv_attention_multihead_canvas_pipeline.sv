...
module qkv_attention_multihead_canvas_pipeline #(
    ...
    qkv_attention_head_pipeline #(
        .INTERNAL_MAC(0), .LUT_FILE(LUT_FILE)
    ) head_pipeline (
    ...
    initial begin
        if (HEADS < 1 || HEADS > 12)
            $error("QKV attention canvas supports one through twelve heads");
    end
