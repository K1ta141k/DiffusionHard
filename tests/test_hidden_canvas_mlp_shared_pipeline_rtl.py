...
def test_hidden_canvas_norm2_and_mlp_share_one_mac() -> None:
    if shutil.which("iverilog") is None or shutil.which("vvp") is None:
        pytest.skip("iverilog and vvp are required")
    build = Path("/tmp/tb_hidden_canvas_mlp_shared_pipeline")
    sources = [
        ...
        "tb_hidden_canvas_mlp_shared_pipeline.sv",
    ]
    compile_result = subprocess.run(
        [
            "iverilog", "-g2012", "-Wall", "-s",
            "tb_hidden_canvas_mlp_shared_pipeline", "-o", str(build),
    ...
    assert "tb_hidden_canvas_mlp_shared_pipeline: PASS" in run_result.stdout
