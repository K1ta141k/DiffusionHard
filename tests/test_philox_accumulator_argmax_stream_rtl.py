...
def test_philox_accumulator_argmax_stream_rtl() -> None:
    iverilog = shutil.which("iverilog")
    vvp = shutil.which("vvp")
    if iverilog is None or vvp is None:
        pytest.skip("Icarus Verilog is not installed")

    build = ROOT / ".pytest-cache-philox-accumulator-argmax-stream-rtl"
    sources = [
        ...
        ROOT / "rtl/output_head/dual_requantizer_q20.sv",
        ROOT / "rtl/output_head/philox_accumulator_argmax_stream.sv",
        ROOT / "rtl/output_head/tb_philox_accumulator_argmax_stream.sv",
    ]
    subprocess.run(
        [
            iverilog,
            "-g2012",
            "-s",
            "tb_philox_accumulator_argmax_stream",
            "-o",
            str(build),
