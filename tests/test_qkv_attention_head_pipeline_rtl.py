...
def test_qkv_projection_rotary_and_attention_share_one_physical_array() -> None:
    if shutil.which("iverilog") is None or shutil.which("vvp") is None:
        pytest.skip("iverilog and vvp are required")
    build = Path("/tmp/tb_qkv_attention_head_pipeline")
    sources = [
        ...
        "tb_qkv_attention_head_pipeline.sv",
    ]
    compile_result = subprocess.run(
        [
            "iverilog", "-g2012", "-Wall", "-s", "tb_qkv_attention_head_pipeline",
    ...
    assert "tb_qkv_attention_head_pipeline: PASS" in run_result.stdout
    match = re.search(r"cycles=(\d+)", run_result.stdout)
    assert match is not None
    assert int(match.group(1)) == 29703
    profile = re.search(
        r"PROFILE staging=(\d+) head=(\d+) qkv_projection=(\d+) "
        r"rotary=(\d+) qk=(\d+) softmax=(\d+) pv=(\d+)",
        run_result.stdout,
    )
