...
def _dynamic_packed_attention_head_q12(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
) -> torch.Tensor:
    ...
    return _symmetric_round_shift(probabilities @ value, 16).clamp(
        -(1 << 17), (1 << 17) - 1
    )


@pytest.mark.parametrize("packed_m8", [False, True], ids=["fixed18", "packed-m8"])
def test_connected_qkv_rotary_attention_head_matches_h0(
    tmp_path: Path,
    packed_m8: bool,
) -> None:
    ...
    _, fixed_attention_q12_values, _ = fixed_attention_q12(
    ...
    if packed_m8:
        value_q12 = rotary_details["tensors"]["qkv_q12"][:, 2, 0]
        expected_attention_q12 = _dynamic_packed_attention_head_q12(
            query_q12[:, 0], key_q12[:, 0], value_q12
        )
    else:
        expected_attention_q12 = fixed_attention_q12_values
    ...
    normalized_hex = tmp_path / "connected_normalized_q12.hex"
    ...
    for kind in range(3):
        for channel_tile in range(11):
            tile_rows.append(
                [
                    kind * 768 + channel
                    if (channel := channel_tile * 6 + lane) < 64
                    else None
                    for lane in range(6)
                ]
            )

    weight_hex = tmp_path / "connected_qkv_weights.hex"
    ...
    multiplier_hex = tmp_path / "connected_qkv_multipliers.hex"
    ...
    bias_hex = tmp_path / "connected_qkv_biases.hex"
    bias_hex.write_text(
        "\n".join(
            _packed_hex(
    ...
    constants_hex = tmp_path / "connected_rotary_constants.hex"
    ...
    expected_hex = tmp_path / "connected_attention_head0.hex"
    expected_hex.write_text(
        "\n".join(
            f"{int(expected_attention_q12[token, channel]) & 0x3FFFF:05x}"
            for token in range(64)
    ...
    expected_qkv_hex = tmp_path / "connected_qkv_head0.hex"
    qkv_view = fixed_qkv_q12.view(64, 3, 12, 64)
    expected_qkv_hex.write_text(
        "\n".join(
            f"{int(qkv_view[token, kind, 0, channel]) & 0x3FFFF:05x}"
            for kind in range(3)
            for token in range(64)
            for channel in range(64)
        )
    ...
    dut_module = (
        ...
        if packed_m8
        else "qkv_attention_head_pipeline"
    )
    packed_dut_ports = ""
    if packed_m8:
        packed_dut_ports = """
...
"""
    testbench = tmp_path / "tb_qkv_attention_head_h0_connected.sv"
    testbench.write_text(
        f"""`timescale 1ns/1ps
module tb_qkv_attention_head_h0_connected;
  ...
  reg [3071:0] weight_mem [0:791];
  reg [143:0] multiplier_mem [0:32];
  reg [107:0] bias_mem [0:32];
  reg [31:0] constant_mem [0:2047];
  reg [17:0] expected_mem [0:4095];
  reg [17:0] expected_qkv_mem [0:12287];
  integer index,tile_count=0,token_lane,lane,token,channel,cycles=0;

  always @* tile_index=requested_kind*11+requested_channel_tile;
  {dut_module} #(.INTERNAL_MAC(0)) dut(
    .clk(clk),.rst_n(rst_n),.start(start),.start_ready(start_ready),.head_in(0),
    .metadata_valid(busy),.metadata_ready(metadata_ready),.metadata_head(0),
    ...
    .weight_tile_valid(busy),.weight_tile_ready(weight_ready),.weight_head(0),
    .weight_kind(requested_kind),.weight_channel_tile(requested_channel_tile),
...
{packed_dut_ports}
    ...
    .staging_busy(staging_busy),.attention_busy(attention_busy),
    .busy(busy),.done(done));
...
{shared_mac}
  ...
  always @(posedge clk) begin
    ...
    #1;
    if(dut.staging.router_query_load &&
       $signed(dut.staging.router_load_q12)!==
       $signed(expected_qkv_mem[dut.staging.router_load_token*64+
                                dut.staging.router_load_channel]))
      $fatal(1,"connected Q mismatch token %0d channel %0d actual %0d expected %0d",
             dut.staging.router_load_token,dut.staging.router_load_channel,
             $signed(dut.staging.router_load_q12),
             $signed(expected_qkv_mem[dut.staging.router_load_token*64+
                                      dut.staging.router_load_channel]));
    if(dut.staging.router_key_load &&
       $signed(dut.staging.router_load_q12)!==
       $signed(expected_qkv_mem[4096+dut.staging.router_load_token*64+
                                dut.staging.router_load_channel]))
      $fatal(1,"connected K mismatch token %0d channel %0d",
             dut.staging.router_load_token,dut.staging.router_load_channel);
    if(dut.staging.router_value_load &&
       $signed(dut.staging.router_load_q12)!==
       $signed(expected_qkv_mem[8192+dut.staging.router_load_token*64+
                                dut.staging.router_load_channel]))
      $fatal(1,"connected V mismatch token %0d channel %0d",
             dut.staging.router_load_token,dut.staging.router_load_channel);
    if(attention_valid) begin
      if(attention_group!==(tile_count/11) ||
         attention_output_tile!==(tile_count%11))
        $fatal(1,"connected attention tag mismatch");
      for(token_lane=0;token_lane<4;token_lane=token_lane+1)
        for(lane=0;lane<attention_valid_channels;lane=lane+1) begin
          token=attention_group*4+token_lane;
          channel=attention_output_tile*6+lane;
          if($signed(attention_data[(token_lane*6+lane)*18 +: 18])!==
             $signed(expected_mem[token*64+channel]))
            $fatal(1,"connected attention mismatch token %0d channel %0d",
                   token,channel);
        end
  ...
  initial begin
    ...
    $readmemh("{expected_qkv_hex}",expected_qkv_mem);
    ...
    if(tile_count!=176) $fatal(1,"missing connected attention tiles %0d",tile_count);
    if(busy || staging_busy || attention_busy)
      $fatal(1,"connected real head remained busy");
    $display("tb_qkv_attention_head_h0_connected: PASS cycles=%0d",cycles);
    $finish;
  end
...
""",
    ...
    build = tmp_path / "tb_qkv_attention_head_h0_connected"
    sources = [
    ...
    compile_result = subprocess.run(
        [
            "iverilog", "-g2012", "-s", "tb_qkv_attention_head_h0_connected",
    ...
    assert "tb_qkv_attention_head_h0_connected: PASS" in run_result.stdout
    match = re.search(r"cycles=(\d+)", run_result.stdout)
    assert match is not None
    expected_cycles = 25879 if packed_m8 else 29527
    assert int(match.group(1)) == expected_cycles
